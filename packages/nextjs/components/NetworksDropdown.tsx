...
export const NetworksDropdown = ({ value, onChange }: { value: number; onChange: (options: any) => any }) => {
  ...
  const defaultNetwork = networks.find(network => network.id === value);

  const defaultValue = defaultNetwork
    ? {
        value: defaultNetwork.id,
        label: defaultNetwork.name,
        icon: defaultNetwork.icon,
      }
    : groupedOptions["mainnet"].options[0];
  ...
  return (
    <Select
      ...
      className="text-sm w-44 max-w-xs bg-white text-neutral-content relative"
      theme={theme => ({
        ...theme,
        colors: {
          ...theme.colors,
